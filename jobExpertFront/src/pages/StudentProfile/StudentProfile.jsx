...
const StudentProfile = () => {
  ...
  const imageLoad = async () => {
    ...
    try {
      if (data.data.avatar) {
        const mx = {
          ...userDa.userData.userInfo, userImg: data.data.avatar
        }
      ...
      <div className="grid relative grid-cols-1 md:grid-cols-3 gap-5 justify-items-center items-center mt-16 bg-secondary p-10 w-4/5 mx-auto">
        ...
        {hide && <div className=" w-[5%]   absolute z-20 shadow-2xl left-[11%] border-gray-700  ">
          <div className="grid grid-cols-1 relative">
            ...
            <div
              onClick={imageLoad}
              className="text-center  w-full text-sm font-sans ml-[50%]  rounded-full  shadow-2xl  cursor-pointer text-secondary font-semibold  absolute bottom-[-10px]"
            >
              Upload
            </div>
          </div>
